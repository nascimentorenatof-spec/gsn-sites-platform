...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}html{scroll-behavior:smooth}body{margin:0;font-family:Arial,sans-serif;color:var(--ink);background:#fff}a{color:inherit}
    .site-nav{position:sticky;top:0;z-index:10;display:flex;align-items:center;justify-content:space-between;gap:20px;padding:16px 44px;background:rgba(255,255,255,.94);border-bottom:1px solid #dfe5e1;backdrop-filter:blur(10px)}
    .site-nav strong{font-size:18px}.site-nav div{display:flex;gap:8px;flex-wrap:wrap;justify-content:flex-end}.site-nav a{padding:8px 10px;border-radius:8px;text-decoration:none;font-size:14px;font-weight:800;color:var(--ink)}
    ...
    section{scroll-margin-top:78px;padding:72px 44px}.grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:18px}.card{border:1px solid #dfe5e1;border-radius:8px;padding:24px;background:#fff}
    ...
    @media(max-width:760px){.site-nav{position:static;align-items:flex-start;flex-direction:column;padding:14px 18px}.site-nav div{justify-content:flex-start}.hero,section{padding:28px 18px}.grid,.about,.contact{grid-template-columns:1fr}.about img{height:280px}}
  </style>
</head>
<body>
  <nav class="site-nav" aria-label="Navegacao do site"><strong>${escapeHtml(form.businessName)}</strong><div>${navigationItems.map(([href, label]) => `<a href="#${href}">${label}</a>`).join("")}</div></nav>
  <main>
    <section id="inicio" class="hero"><div><p class="eyebrow">${escapeHtml(form.segment)} | ${escapeHtml(form.visualStyle)}</p><h1>${escapeHtml(content.heroTitle)}</h1><p>${escapeHtml(content.heroSubtitle)}</p><a class="button" href="#contato">${escapeHtml(content.primaryCta)}</a></div></section>
    <section id="diferenciais"><p class="eyebrow">Diferenciais</p><h2>${escapeHtml(content.proofTitle)}</h2><p>${escapeHtml(content.proofText)}</p><div class="grid">${benefits.map((benefit) => `<article class="card"><h3>${escapeHtml(benefit)}</h3><p>Informacao direta para ajudar o visitante a decidir com seguranca.</p></article>`).join("")}</div></section>
    <section id="sobre" class="about"><div><p class="eyebrow">Sobre</p><h2>${escapeHtml(content.aboutTitle)}</h2><p>${escapeHtml(content.aboutText)}</p></div><img src="${secondImage}" alt="${escapeHtml(form.businessName)}"></section>
    <section id="servicos"><p class="eyebrow">Servicos</p><h2>O que voce encontra aqui</h2><div class="grid">${services.map((service) => `<article class="card"><h3>${escapeHtml(service)}</h3><p>Atendimento pensado para resolver com clareza e agilidade.</p></article>`).join("")}</div></section>
    <section id="depoimentos"><p class="eyebrow">Depoimentos</p><h2>Confianca para dar o proximo passo</h2><div class="grid">${testimonials.map((testimonial) => `<article class="card"><h3>${escapeHtml(testimonial.name)}</h3><p>${escapeHtml(testimonial.quote)}</p></article>`).join("")}</div></section>
    <section id="faq"><p class="eyebrow">Duvidas frequentes</p><h2>Perguntas comuns</h2><div class="grid">${faq.map((item) => `<article class="card"><h3>${escapeHtml(item.question)}</h3><p>${escapeHtml(item.answer)}</p></article>`).join("")}</div></section>
