...
async function createPagSeguroCheckout(project: SiteProject): Promise<CheckoutResult> {
  ...
  const checkoutLink = payload.links?.find((l) => l.rel === "PAY" || l.rel === "SELF")?.href;
  if (!checkoutLink || !payload.id) {
    throw new Error("PagSeguro nao retornou link de checkout valido.");
  }
  return { provider: "pagseguro", checkoutUrl: checkoutLink, reference: payload.id };
}
